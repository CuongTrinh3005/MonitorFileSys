...
class SavingsAccountTestCase(SmartContracts340TestCase):
    ...
    filepath = os.environ.get(
        'DATA_SAVINGS_V340',
        'contracts_sdk/example_unit_tests/smart_contracts/savings_v340.py'
    )
    ...
    key_date = types.Parameter(
        ...
        shape=types.OptionalShape(types.NumberShape(
            min_value=1,
            max_value=31,
            step=1,
        )),
        ...
        self.assertEqual(
            (
                ('APPLY_ACCRUED_INTEREST', {
                    'day': str(self.key_date.value.value),
                    'hour': '0',
                    'minute': '1'
                }),
                ('ACCRUE_INTEREST', {
    ...
    def test_post_parameter_change_code_does_not_amend_schedule_if_key_date_not_updated(self):
        self.run_contract_function(
            self.contract_code, 'post_parameter_change_code', self.old_parameter_values, {},
            self.effective_date
        )
    ...
    def test_post_parameter_change_code_amends_schedule(self):
        ...
        self.run_contract_function(
            self.contract_code, 'post_parameter_change_code', self.old_parameter_values,
            updated_parameter_values, self.effective_date
        )
