...
class PostingInstruction:
    ...
    def __init__(self, **kwargs):
        self._from_proto = kwargs.pop('_from_proto', False)
        ...
        if not self._from_proto:
            self._spec().assert_constructor_args(
                self._registry, instruction_kwargs)

        for name, value in kwargs.items():
            setattr(self, name, value)

    def __setattr__(self, name, value):
        if name == '_vault' and not value:
            return
        elif (
            name not in ['_vault', '_from_proto', '_batch_id', 'balances'] and
            not self._from_proto
        ):
...
class PostingInstructionBatch(types_utils.TypedList('PostingInstruction')):
    ...
    def __init__(
            self, *, batch_details, client_batch_id, value_timestamp, batch_id=None,
            client_id=None, posting_instructions=(), _from_proto=False
    ):
    ...
    def __setattr__(self, name, value):
        if name not in ['_from_proto', 'balances'] and not self._from_proto:
            self._spec().assert_attribute_value(self._registry, name, value)
        super().__setattr__(name, value)
...
class ClientTransactionEffects:
    ...
    def __init__(
        self, *, authorised=Decimal(0), released=Decimal(0),
        settled=Decimal(0), unsettled=Decimal(0), _from_proto=False
    ):
        self._from_proto = _from_proto
        kwargs = {
            'authorised': authorised, 'released': released,
    ...
    def __setattr__(self, name, value):
        if name != '_from_proto' and not self._from_proto:
            self._spec().assert_attribute_value(self._registry, name, value)
        super().__setattr__(name, value)
    ...
    def __eq__(self, other):
        return (
            ...
            self.authorised == other.authorised and
            self.released == other.released and
            self.settled == other.settled and
...
class ClientTransactionEffectsDefaultDict(
    types_utils.TypedDefaultDict(_effects_item_type)
):
