...
def scheduled_code(event_type, effective_date):
    ...
    accrue_balance = sum(
        balance.net
        for (address, asset, denom, phase), balance in
        vault.get_balance_timeseries().latest().items()
        if phase in (Phase.PENDING_OUT, Phase.COMMITTED) and
        address in (DEFAULT_ADDRESS, 'ACCRUED_INCOMING', 'ACCRUED_OUTGOING') and
        denom == contract_denomination and asset == DEFAULT_ASSET
    )
    posting_ins = None
    if accrue_balance > 0:
        interest_due = (
            Decimal(monthly_interest_rate) * accrue_balance
        ).copy_abs().quantize(Decimal('.01'), rounding=ROUND_HALF_DOWN)
        if interest_due > 0:
            posting_ins = vault.make_internal_transfer_instructions(
                amount=interest_due, denomination=contract_denomination,
                from_account_id='1', from_account_address='ACCRUED_OUTGOING',
                to_account_id=vault.account_id, to_account_address='ACCRUED_INCOMING',
                asset=DEFAULT_ASSET, client_transaction_id='INTEREST_{}'.format(vault.account_id)
            )
    ...
    # Charge any fees for going into the overdraft.
        ...
        if fees_due < 0:
            posting_ins = vault.make_internal_transfer_instructions(
                amount=fees_due, denomination=contract_denomination,
                from_account_id='1', from_account_address='ACCRUED_OUTGOING',
                to_account_id=vault.account_id, to_account_address='ACCRUED_INCOMING',
                asset=DEFAULT_ASSET, client_transaction_id='INTEREST_{}'.format(vault.account_id)
            )
...
def derived_parameters(effective_date):
    ...
    if _denomination not in permitted_denominations:
        raise Rejected(message='Invalid denominations used within this contract', reason_code=RejectedReason.AGAINST_TNC)
...
def pre_posting_code(postings, effective_date):
    if len(postings) == 1:
        ...
        posting = postings[0]
        if (
                posting.client_batch_id != '123' or
                posting.value_timestamp != effective_date or
                posting.insertion_timestamp != effective_date or
                posting.batch_details != {} or
                posting.client_id != '1' or
                posting.batch_id != '2'
        ):
            raise Rejected(
                message='Not all PI attributes are set',
        ...
        if posting.balances()[
            ...
            raise Rejected(
                message='Unexpected default PI balances',
                reason_code=RejectedReason.AGAINST_TNC
            )
        # Check that PIB balances method can be mocked.
        default_address = vault.get_balance_timeseries().latest()[
            (DEFAULT_ADDRESS, DEFAULT_ASSET, 'GBP', Phase.COMMITTED)
        ].net
    ...
    if len(postings) == 2:
        ...
        if (ct.effects()[(DEFAULT_ADDRESS, DEFAULT_ASSET, 'GBP')].settled != Decimal(10) or
            ct.effects()[(DEFAULT_ADDRESS, DEFAULT_ASSET, 'GBP')].authorised != Decimal(20) or
            ct.effects()[(DEFAULT_ADDRESS, DEFAULT_ASSET, 'GBP')].released != Decimal(0) or
            ct.effects()[(DEFAULT_ADDRESS, DEFAULT_ASSET, 'GBP')].unsettled != Decimal(10)
        ):
            raise Rejected(
    ...
    calendar_events = vault.get_calendar_events(
        calendar_ids=['date_A', 'date_B']
    )
    if len(calendar_events) != 2:
        raise Rejected(
            'Wrong number of calendar events found',
            reason_code=RejectedReason.AGAINST_TNC
        )
    for event in calendar_events:
        if (event.start_timestamp.year != 2020 or
            event.start_timestamp.month != 1 or
            event.start_timestamp.day != 1):
            raise Rejected(
                'Calendar event start_timestamp incorrect',
                reason_code=RejectedReason.AGAINST_TNC
            )
