...
def _precision_accrual(vault, amount):
    try:
        ...
        _validate_positive_int(digits, min_val=0, max_val=_DEFAULT_ACCRUE_DIGITS, name='Decimal for accrue')
    except:
...
def _precision_apply(vault, amount):
    try:
        ...
        _validate_positive_int(digits, min_val=0, max_val=int(_DEFAULT_APPLY_DIGITS) + 1, name='Decimal for accrue')
    except:
