...
class SimpleTestCase(ContractModules390TestCase):
    ...
    def test_round_accrual(self):
        ...
        amount_rounded = self.run_contract_function(
            self.contract_code, 'round_accrual', amount
        )
    ...
    def test_round_fullfilment(self):
        amount = Decimal('0.123456789')
        amount_rounded = self.run_contract_function(
            self.contract_code, 'round_fulfilment', amount
        )
    ...
    def test_build_posting_instruction_batch_fails_with_negative_amount(self):
        ...
        self.assertIn(
            'Cannot build PostingInstructions with negative amount',
            str(ex.exception)
        )
