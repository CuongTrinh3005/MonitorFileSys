...
class ContractsTestCase(TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.builtins = cls._contract_lib.WHITELISTED_BUILTINS
    ...
    def create_sandbox(self, types):
        return {
            '__builtins__': {name: self._registry['__builtins__'][name] for name in self.builtins},
            **types,
        }

    def run_contract_function(self, contract_code, function_name, *args, **kwargs):
...
class SmartContractsTestCase(ContractsTestCase):
    ...
    supported_hook_names = [
        'pre_posting_code', 'post_posting_code', 'post_activate_code', 'execution_schedules',
        'close_code', 'upgrade_code', 'pre_parameter_change_code', 'post_parameter_change_code',
        'derived_parameters', 'scheduled_code'
    ]
...
class SupervisorContractsTestCase(ContractsTestCase):
    ...
    supported_hook_names = [
        'post_posting_code', 'execution_schedules', 'scheduled_code'
    ]
    ...
    def setUpClass(cls):
        ...
        cls._supervisee_contract_lib = importlib.import_module(
            supervisee_path, versions_package
        )
