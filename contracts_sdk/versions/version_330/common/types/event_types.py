...
class EventTypesGroup:
    ...
    def __init__(self, *, name, event_types_order):
        self._spec().assert_constructor_args(
            self._registry, {
                'name': name,
                'event_types_order': event_types_order
            }
        )
        ...
        return types_utils.ClassSpec(
            ...
            docstring='''
                **Only available in version 3.3+.**
                A group of event types that defines the scheduling order for events in the group.
                If any event type groups are defined in a Smart Contract, a list of
                [EventType](#classes-EventType)s must also be defined. Every event type from a
                group must be defined in the event types list. Event type groups are unique for
                each account. Events associated with different accounts are **NOT** added to the
                same group even if the event type group names match.
            ''',
