...
class SimpleTestCase(SupervisorContracts380TestCase):
    ...
    filepath = os.environ.get(
        'DATA_SIMPLE_V380',
        'contracts_sdk/example_unit_tests/supervisor_contracts/simple_v380.py'
    )
    ...
    def test_scheduled_code_with_calendar_event_type(self):
        ...
        def get_calendar_events(calendar_ids):
            ...
            if calendar_ids == ['A']:
                calendar_events = types.CalendarEvents(
                    calendar_events=calendar_events_a
                )
        ...
        self.run_contract_function(
            self.contract_code, 'scheduled_code',
    ...
    def test_post_posting_code(self):
        ...
        self.run_contract_function(
            self.contract_code, 'post_posting_code',
        ...
        self.vault.supervisees[
            'supervisee'
        ].add_account_note.assert_called_once_with(
            body=(
                f'Successfully created an account note for {hook_postings[0].account_id}'
            ),
            note_type=types.NoteType.RAW_TEXT,
            is_visible_to_customer=True,
            date=self.effective_date
