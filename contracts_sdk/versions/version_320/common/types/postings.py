...
from ....version_310.common.types import (
    postings as postings310, PostingInstructionType
)
    ...
    def __init__(self, **kwargs):
        if not kwargs.get('_from_proto', False):
            self._spec().assert_constructor_args(
                self._registry,
                {'advice': kwargs.get('advice', False)}
            )
            ...
            if type_arg not in [
                ...
                if advice_arg is not None:
                    raise StrongTypingError(f'Advice can not be set for {type_arg}')
