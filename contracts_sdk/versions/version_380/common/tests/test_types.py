...
from ..types import (
    AddAccountNoteDirective, AmendScheduleDirective, EventTypeSchedule, HookDirectives,
    NoteType, Phase, PostingInstruction, PostingInstructionBatch, PostingInstructionBatchDirective,
    PostingInstructionType, RemoveSchedulesDirective, UpdateAccountEventTypeDirective,
...
class PublicCommonV380TypesTestCase(PublicCommonV370TypesTestCase):
    ...
    def test_event_type_schedule_can_be_created(self):
        event_type_schedule = EventTypeSchedule(
            day='day',
            year='year'
        )
    ...
    def test_event_type_schedule_empty_schedule(self):
        with self.assertRaises(StrongTypingError) as ex:
            EventTypeSchedule()
        self.assertIn(
            'Empty EventTypeSchedule object created',
            str(ex.exception)
        )
    ...
    def test_event_type_schedule_attributes_are_verified(self):
        ...
        self.assertIn(
            "'day' expected Optional[str] but got value 1",
            str(ex.exception)
        )
    ...
    def test_hook_directives(self):
                    ...
                    posting_instruction_batch=PostingInstructionBatch(
                        ...
                        posting_instructions=[
                            PostingInstruction(
                                ...
                                denomination='GBP',
                                account_id=self.account_id_380,
                                account_address=symbols.DEFAULT_V3_ADDRESS,
                                asset=symbols.DEFAULT_V3_ASSET,
        ...
        self.assertEqual(
            self.request_id_380,
            hook_directives.add_account_note_directives[0].idempotency_key
        )
        self.assertEqual(
            self.request_id_380,
            hook_directives.amend_schedule_directives[0].request_id
        )
        self.assertEqual(
            self.request_id_380,
            hook_directives.remove_schedules_directives[0].request_id
        )
        self.assertEqual(
            self.request_id_380,
            hook_directives.posting_instruction_batch_directives[0].request_id
        )
        self.assertEqual(
            self.request_id_380,
            hook_directives.workflow_start_directives[0].idempotency_key
        )
        self.assertEqual(
            self.account_id_380,
            hook_directives.update_account_event_type_directives[0].account_id
        )
