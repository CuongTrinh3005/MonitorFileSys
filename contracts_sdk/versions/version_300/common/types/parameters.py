...
class Parameter:
    ...
    def __init__(self, **kwargs):
        if (
                kwargs['level'] == ContractParameterLevel.INSTANCE and
                kwargs.get('default_value', None) is None and
                not issubclass(kwargs.get('shape', None), OptionalShape)
        ):
            name = kwargs['name']
    ...
    def _public_attributes(cls, language_code=symbols.Languages.ENGLISH):
        ...
        return [
            ...
            types_utils.ValueSpec(
                name='level',
                type='Level',
                docstring='The level of the Parameter.'
            ),
            types_utils.ValueSpec(
                name='value',
            ...
            types_utils.ValueSpec(
                name='default_value',
                type='Optional[%s]' % _parameter_value_type_str,
                docstring='The default value of the Parameter. Deprecated.'
            ),
            types_utils.ValueSpec(
...
class Shape:
    ...
    def __new__(cls, **kwargs):
        ...
        if missing_kwargs:
            raise utils_exceptions.InvalidSmartContractError('%s does not have attribute%s %s' % (
                cls.__name__,
                's' if len(missing_kwargs) > 1 else '',
                ', '.join(repr(missing_kwarg) for missing_kwarg in missing_kwargs)
            ))
...
class NumberShape(Shape):
    ...
    def __new__(cls, **kwargs):
        ...
        attrs = set(cls._attribute_names())  # noqa: SLF001
        attrs.remove('kind')
        for attr in attrs:
    ...
    def _spec(cls, language_code=symbols.Languages.ENGLISH):
        ...
        return types_utils.ClassSpec(
            name='NumberShape',
            docstring='A Parameter shape defining a floating-point number.',
            constructor=types_utils.ConstructorSpec(
        ...
        return types_utils.ClassSpec(
            ...
            public_attributes=[
                types_utils.ValueSpec(
                    name='value',
                    type='Any',
                    docstring='The value, if specified.'
                )
            ],
            constructor=types_utils.ConstructorSpec(
                docstring='',
...
class ParameterTimeseries(
        types_utils.Timeseries(_parameter_value_type_str, 'parameter value')):
    ...
    def _spec(cls, language_code=symbols.Languages.ENGLISH):
        ...
        return types_utils.merge_class_specs(
            derived_spec=types_utils.ClassSpec(
                ...
                docstring='A timeseries of Parameter objects.'
            ),
